...
for i = 0:((M1 - 1)*2)
    X_p = fft(x(((N/(M1*2)*i) + 1):((N/(M1*2)*(i + 2))))*w'); % FFT delle sottosequenze
    for j = 1:(N/M1)
...
for i = 0:((M2 - 1)*2)
    X_p = fft(x(((N/(M2*2)*i) + 1):((N/(M2*2)*(i + 2))))*w'); % FFT delle sottosequenze
    for j = 1:(N/M2)
...
for i = 0:((M3 - 1)*2)
    X_p = fft(x(((N/(M3*2)*i) + 1):((N/(M3*2)*(i + 2))))*w'); % FFT delle sottosequenze
    for j = 1:(N/M3)
