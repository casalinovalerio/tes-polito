...
ntot=15*10^4;   % Numero totale di campioni
N=15*10^3;    % Numero di campioni diversi da zero

x=zeros(1,ntot); %%% preallocazione segnale nullo

configBit = 1; %0/1 1 per la prima parte dell'esercizio, 0 per la seconda
if(configBit)
    x(1:N)=ones(1,N);
else
    seed=1;
